...
def register(employe: EmployeCreate, db: Session = Depends(get_db)):
    """Inscription d'un nouvel utilisateur. Crée un profil Technicien si le rôle correspond."""
...
def login(employe_credentials: EmployeLogin, db: Session = Depends(get_db)):
    """Connexion classique par mot de passe (Employé / Technicien)."""
...
async def request_login_otp(email: str, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    """Génère et envoie un code OTP pour la connexion Admin."""
    ...
    background_tasks.add_task(send_otp_login_email, to_email=employe.email, first_name=employe.first_name, otp=otp)
...
def verify_login_otp(email: str, otp: str, db: Session = Depends(get_db)):
    """Vérifie le code OTP et délivre le jeton JWT."""
    employe = db.query(Employe).filter(Employe.email == email).first()
    if not employe:
...
async def forgot_password(email: str, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    # SÉCURITÉ : L'admin n'a pas de mot de passe, il ne peut pas le réinitialiser
    if employe.role == RoleEnum.ADMIN:
        raise HTTPException(status_code=403, detail="Les administrateurs n'utilisent pas de mot de passe. Veuillez utiliser la connexion par code OTP.")
...
def get_me(current_user: Employe = Depends(get_current_user)):
    """Récupère le profil de l'utilisateur connecté via le jeton JWT."""
    return current_user
